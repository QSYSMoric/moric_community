<template>
  <v-container fluid class="home-article">
    <v-row>
      <v-col :cols="12" :md="10" class="p-t-0!">
        <v-infinite-scroll class="p-x-4px p-t-0!" mode="manual" @load="load">
          <template v-for="(item, index) of trends" class="mb-4" :key="index">
            <v-card
          ...
          <template v-slot:empty>
            <v-alert type="warning">没有更多动态了!</v-alert>
          </template>
        </v-infinite-scroll>
      </v-col>
      <!-- <v-col :cols="12" :md="3" class="d-none d-md-block d-xl-none">
        <v-card class="mx-auto">
          <v-card-text>各个文章所对应的热度话题讨论度</v-card-text>
        </v-card>
      </v-col> -->
    </v-row>
  </v-container>
...
function load({ done }: { done: Function }): void {
  // done("empty");
  page.value++;
  getHomeTrends(page.value).then((data) => {
    if (data.data.length) {
      data.data.forEach((element: TrendHome) => {
        trends.value.push(new Trend(element));
        done("ok");
      });
    } else {
      done("empty");
